...
| Retrieval | Local knowledge import, chunk inspection, and grounded query validation. |
| Experiments | Unified session/run timeline, artifact lineage, navigation, filters, and retention. |
...
| `v0.4` | Foreground `/fine-tune`, `/compare`, `/models`, `/benchmarks`, `/retrieval`, and `/experiments`; feature-owned state/actions; artifact lineage and retention; dark-glass studio/workbench style; canonical APIs with deprecated Admin compatibility wrappers; GitHub/ModelScope synchronization. |
...
- Current tagged release note: [v0.4.0](https://github.com/ChrisChen667788/local-agent-lab/blob/main/docs/releases/v0.4.0_2026-06-12.md)
...
| Retrieval | 本地知识导入、chunk 检查和 grounded query 验证。 |
| Experiments | 统一 Session/Run 时间线、artifact lineage、导航、筛选和保留策略。 |
...
| `v0.4` | 前台 `/fine-tune`、`/compare`、`/models`、`/benchmarks`、`/retrieval`、`/experiments`；feature-owned state/actions；artifact lineage 与 retention；dark-glass studio/workbench 风格；canonical API 与带弃用头的 Admin compatibility wrappers；GitHub/ModelScope 同步。 |
...
- 当前已打标签版本说明: [v0.4.0](https://github.com/ChrisChen667788/local-agent-lab/blob/main/docs/releases/v0.4.0_2026-06-12.md)
