...
import {
  mutateOperationalExceptionLifecycleApplication,
  readOperationalExceptionLifecycleApplication,
...
export async function GET() {
  try {
    return NextResponse.json(readOperationalExceptionLifecycleApplication());
  } catch (error) {
    return NextResponse.json({
      ...
      error: error instanceof Error ? error.message : "Operational exception lifecycle evidence could not be read.",
    }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, ...mutateOperationalExceptionLifecycleApplication(body) }, { status: 201 });
  } catch (error) {
    ...
    return NextResponse.json({
      ...
      externalStatus: "hold",
      productionStatus: "blocked",
      error: error instanceof Error ? error.message : "Operational exception lifecycle action failed.",
    }, { status });
  }
}
