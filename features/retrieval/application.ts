...
export async function POST(request: Request) {
  try {
    ...
    if (body.importMode === "path" || body.importMode === "path-probe") {
      ...
      const importId = `retrieval-import-${crypto.randomUUID()}`;
      appendExperimentEvent({
        kind: "retrieval",
        status: "completed",
        title: "Knowledge path imported",
        summary: `${imported.length} document${imported.length === 1 ? "" : "s"} indexed from ${inspection.kind}`,
        relatedId: importId,
        artifacts: [
          {
        ...
        links: imported.slice(0, 12).map((document) => ({
        ...
        metadata: {
          ...
          recursive: inspection.recursive,
    ...
    appendExperimentEvent({
      ...
      summary: `${result.document.title} · ${result.document.chunkCount} chunks`,
      relatedId: result.document.id,
      artifacts: result.document.source
        ? [
            {
              kind: /^https?:\/\//i.test(result.document.source) ? "url" : "file",
              role: "input",
      ...
      metadata: {
        ...
        characterCount: result.document.charCount,
      },
    });
...
export async function DELETE(request: Request) {
  ...
  appendExperimentEvent({
    kind: "retrieval",
    status: "cancelled",
    title: "Knowledge document removed",
    summary: `Removed document ${id} from the retrieval index.`,
    relatedId: id,
    links: [
      { relation: "source", entityType: "document", id },
    ],
  });
