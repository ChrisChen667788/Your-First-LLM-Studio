...
export type AgentGetCodePanelProps = {
  locale: string;
  open: boolean;
  mode: "chat" | "compare";
        ...
        <div className="flex flex-col gap-4 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="inline-flex rounded-full border border-white/10 bg-black/25 p-1">
              {(Object.keys(LANGUAGE_LABELS) as AgentReproduceLanguage[]).map(
                (item) => (
                  <button
                    key={item}
                    type="button"
            ...
            <button
              type="button"
              onClick={() =>
                onCopy(snippets[language], `get-code:${mode}:${language}`)
              }
          ...
          <pre className="max-h-[56vh] overflow-auto rounded-[24px] border border-white/10 bg-black/30 p-4 text-[13px] leading-6 text-slate-100">
