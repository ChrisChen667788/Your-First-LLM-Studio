...
export function V161ApplicationContractsPanel({ locale }: { locale: string }) {
  ...
  const load = useCallback(async (run = false) => {
    ...
    try {
      const response = await fetch(
        "/api/experiments/v161-application-contracts",
        {
          method: run ? "POST" : "GET",
          cache: "no-store",
      ...
      if (!response.ok && response.status !== 422) {
        throw new Error(
          body.error || "v1.6.1 application contract acceptance failed.",
        );
      }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase text-cyan-300">
            V1.6.1 APPLICATION CONTRACTS
          </p>
          <h2 className="mt-2 text-xl font-semibold text-white">
            {en ? "Canonical product APIs" : "产品级 Canonical API 收口"}
          </h2>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-400">
            {en
              ? "Fifteen checks keep Benchmark lifecycle routes and Compare recipe persistence in their owning feature namespaces while historical Agent and Admin routes remain thin compatibility adapters."
              : "15 项检查确保 Benchmark 生命周期与 Compare recipe 持久化进入各自 feature 命名空间，历史 Agent/Admin 地址只保留薄兼容适配。"}
          </p>
        </div>
