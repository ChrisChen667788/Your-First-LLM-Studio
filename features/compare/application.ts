...
import { prewarmLocalTargetWithRecovery } from "@/features/agent/runtime-prewarm";
...
export async function POST(request: Request) {
  let requestId = "";
  let experimentContext: ExperimentSourceContext | undefined;
  try {
    const body = (await request.json()) as Partial<
      AgentCompareRequest & { experimentContext?: ExperimentSourceContext }
    >;
    ...
    appendExperimentEvent({
      ...
      targetIds,
      artifacts: [
        ...inheritExperimentSourceArtifacts(experimentContext),
        {
          kind: "api",
          role: "progress",
    ...
    appendExperimentEvent({
      ...
      links: [
        ...buildExperimentSourceLinks(experimentContext),
        {
          relation: "produced",
          entityType: "compare",
          id: response.runId,
