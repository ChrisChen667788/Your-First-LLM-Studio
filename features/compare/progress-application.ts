...
export async function GET(request: Request) {
  ...
  if (!progress) {
    return NextResponse.json(
      { error: `No compare progress found for ${requestId}.` },
      { status: 404 },
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (!current) {
      return NextResponse.json(
        { error: `No compare progress found for ${body.requestId}.` },
        { status: 404 },
      );
    }
    if (!current.lanes.some((lane) => lane.targetId === body.targetId)) {
      return NextResponse.json(
        { error: `Target ${body.targetId} is not part of this compare run.` },
        { status: 404 },
      );
    }
    ...
    if (!updated) {
      return NextResponse.json(
        { error: `Failed to update compare progress for ${body.requestId}.` },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to update compare progress.",
      },
      { status: 500 },
    );
  }
