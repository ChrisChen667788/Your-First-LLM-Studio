...
const MODEL_DISCOVERY_API_PATH = "/api/models/discovery";
...
export function ModelDiscoveryPanel({ locale }: ModelDiscoveryPanelProps) {
  const text = useMemo(() => {
  ...
  const loadSummary = useCallback(async () => {
    setPending(true);
    try {
      const response = await fetch(MODEL_DISCOVERY_API_PATH, { cache: "no-store" });
  ...
  async function runScan() {
    ...
    setMessage("");
    try {
      const response = await fetch(MODEL_DISCOVERY_API_PATH, {
        method: "POST",
