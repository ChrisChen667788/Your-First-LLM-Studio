...
check_json_field "Apple release signing readiness" "$BASE_URL/api/desktop/apple-release-signing" "data.ok===true && data.schemaVersion==='desktop.apple-release-signing.v1' && typeof data.ready==='boolean' && data.tools && typeof data.tools.notarytool==='boolean' && Array.isArray(data.blockers)"
...
check_json_field "Desktop v1.1 onboarding release contract" "$BASE_URL/api/desktop/onboarding-release" "data.ok===true && data.schemaVersion==='desktop.onboarding-release.v1' && data.version==='1.1.0-rc.1' && typeof data.localRcReady==='boolean' && typeof data.gaReady==='boolean' && Array.isArray(data.steps) && data.steps.length>=9 && data.totals && Array.isArray(data.gaBlockers) && data.paths && typeof data.paths.releaseManifest==='string'"
...
check_json_field "Experiments release train contract" "$BASE_URL/api/experiments/release-train" "data.ok===true && data.activeVersion==='v1.1.0-rc.1' && Array.isArray(data.milestones) && data.milestones.length===20 && data.milestones.some((item)=>item.version==='v1.0.0' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.1.0' && item.status==='evidence-needed') && data.milestones.some((item)=>item.version==='v1.5.1' && item.status==='planned') && data.milestones.every((item)=>typeof item.version==='string' && Array.isArray(item.scope) && Array.isArray(item.acceptance) && Array.isArray(item.evidence))"
check_json_field "Experiments promotion gate contract" "$BASE_URL/api/experiments/promotion-gate" "data.ok===true && data.schemaVersion==='experiments.promotion-gate.v1' && data.activeVersion==='v1.1.0-rc.1' && ['pass','watch','hold'].includes(data.overallStatus) && Array.isArray(data.sources) && data.sources.length>=5 && data.sources.some((source)=>source.id==='adapter-export') && data.sources.some((source)=>source.id==='docs-screenshots') && Array.isArray(data.blockers) && Array.isArray(data.releaseNoteDraft)"
check_json_field "Experiments release evidence matrix contract" "$BASE_URL/api/experiments/release-evidence-matrix" "data.ok===true && data.schemaVersion==='experiments.release-evidence-matrix.v1' && data.activeVersion==='v1.1.0-rc.1' && Array.isArray(data.rounds) && data.rounds.length===20 && data.totals && data.totals.roundCount===20 && data.totals.plannedCount>=9 && data.rounds.every((round)=>typeof round.version==='string' && typeof round.completionPct==='number' && Array.isArray(round.shipped) && Array.isArray(round.evidence) && Array.isArray(round.blockers) && Array.isArray(round.nextActions))"
