...
check_json_field "Experiments release train contract" "$BASE_URL/api/experiments/release-train" "data.ok===true && data.activeVersion==='v1.5.1' && data.developmentVersion==='v1.7.0-v2.1.9 source and evidence trains' && Array.isArray(data.milestones) && data.milestones.length===65 && data.milestones.some((item)=>item.version==='v1.0.0' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.5.1' && item.status==='blocked') && data.milestones.some((item)=>item.version==='v1.6.7' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.6.8' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.6.9' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.7.0' && item.status==='active') && data.milestones.some((item)=>item.version==='v1.10.1' && item.status==='planned') && data.milestones.some((item)=>item.version==='v1.12.0' && item.status==='planned') && data.milestones.some((item)=>item.version==='v2.0.0' && item.status==='planned') && ['v2.0.1','v2.0.2','v2.0.3','v2.0.4','v2.0.5','v2.1.0','v2.1.1','v2.1.2','v2.1.3','v2.1.4','v2.1.5','v2.1.6','v2.1.7','v2.1.8','v2.1.9'].every((version)=>data.milestones.some((item)=>item.version===version && item.status==='evidence-needed')) && data.milestones.every((item)=>typeof item.version==='string' && Array.isArray(item.scope) && Array.isArray(item.acceptance) && Array.isArray(item.evidence))"
...
check_json_field "Experiments release evidence matrix contract" "$BASE_URL/api/experiments/release-evidence-matrix" "data.ok===true && data.schemaVersion==='experiments.release-evidence-matrix.v1' && data.activeVersion==='v1.5.1' && Array.isArray(data.rounds) && data.rounds.length===65 && data.totals && data.totals.roundCount===65 && data.totals.plannedCount>=19 && data.rounds.filter((round)=>/^v1\\.[1-5]\\./.test(round.version)).every((round)=>round.completionPct>0) && data.rounds.some((round)=>round.version==='v1.6.8' && round.status==='complete') && data.rounds.some((round)=>round.version==='v1.10.1' && round.status==='planned') && data.rounds.some((round)=>round.version==='v1.12.0' && round.status==='planned') && ['v2.0.1','v2.0.2','v2.0.3','v2.0.4','v2.0.5','v2.1.0','v2.1.1','v2.1.2','v2.1.3','v2.1.4','v2.1.5','v2.1.6','v2.1.7','v2.1.8','v2.1.9'].every((version)=>data.rounds.some((round)=>round.version===version && round.status==='evidence-needed')) && data.rounds.every((round)=>typeof round.version==='string' && typeof round.completionPct==='number' && Array.isArray(round.shipped) && Array.isArray(round.evidence) && Array.isArray(round.blockers) && Array.isArray(round.nextActions))"
