...
check_json_field "Models runtime operations contract" "$BASE_URL/api/models/runtime-operations?limit=5" "data.ok===true && data.operations && data.operations.contractVersion==='models.runtime-operations.v2' && Array.isArray(data.operations.capabilities) && data.operations.capabilities.includes('developer-api') && data.operations.registry && Array.isArray(data.operations.registry.profiles) && data.operations.idleUnload && data.operations.requestLogs && Array.isArray(data.operations.requestLogs.entries) && data.operations.developerApi && typeof data.operations.developerApi.chatCompletionsUrl==='string' && Array.isArray(data.operations.targetCards) && data.operations.targetCards.every((card)=>typeof card.targetId==='string' && typeof card.chatCompletionsUrl==='string' && typeof card.profileCount==='number' && typeof card.requestCount==='number')"
