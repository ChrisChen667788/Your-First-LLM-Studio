...
check_json_field "Experiments release train contract" "$BASE_URL/api/experiments/release-train" "data.ok===true && data.activeVersion==='v1.3.0' && Array.isArray(data.milestones) && data.milestones.length===20 && data.milestones.some((item)=>item.version==='v1.0.0' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.1.0' && item.status==='evidence-needed') && data.milestones.some((item)=>item.version==='v1.3.1' && item.status==='complete') && data.milestones.some((item)=>item.version==='v1.5.1' && item.status==='blocked') && data.milestones.every((item)=>typeof item.version==='string' && Array.isArray(item.scope) && Array.isArray(item.acceptance) && Array.isArray(item.evidence))"
check_json_field "Experiments promotion gate contract" "$BASE_URL/api/experiments/promotion-gate" "data.ok===true && data.schemaVersion==='experiments.promotion-gate.v1' && data.activeVersion==='v1.3.0' && ['pass','watch','hold'].includes(data.overallStatus) && Array.isArray(data.sources) && data.sources.length>=5 && data.sources.some((source)=>source.id==='adapter-export') && data.sources.some((source)=>source.id==='docs-screenshots') && Array.isArray(data.blockers) && Array.isArray(data.releaseNoteDraft)"
check_json_field "Experiments release evidence matrix contract" "$BASE_URL/api/experiments/release-evidence-matrix" "data.ok===true && data.schemaVersion==='experiments.release-evidence-matrix.v1' && data.activeVersion==='v1.3.0' && Array.isArray(data.rounds) && data.rounds.length===20 && data.totals && data.totals.roundCount===20 && data.totals.plannedCount===0 && data.rounds.filter((round)=>/^v1\\.[1-5]\\./.test(round.version)).every((round)=>round.completionPct>0) && data.rounds.every((round)=>typeof round.version==='string' && typeof round.completionPct==='number' && Array.isArray(round.shipped) && Array.isArray(round.evidence) && Array.isArray(round.blockers) && Array.isArray(round.nextActions))"
