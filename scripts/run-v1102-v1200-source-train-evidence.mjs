...
const response = await fetch(
  `${baseUrl}/api/experiments/v1102-v1200-source-train`,
  {
    method: "POST",
    headers: {
...
if ((!response.ok && response.status !== 422) || !payload.receipt) {
  throw new Error(
    payload.error || `Future source train acceptance returned HTTP ${response.status}.`,
  );
}

const directory = path.join(root, "output", "release-evidence");
mkdirSync(directory, { recursive: true });
const outputPath = path.join(directory, "v1.10.2-v1.12.0-source-train.json");
writeFileSync(
  outputPath,
  `${JSON.stringify(
    {
      schemaVersion: "experiments.v1102-v1200-source-train-evidence.v1",
