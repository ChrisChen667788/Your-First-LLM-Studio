...
const response = await fetch(
  `${baseUrl}/api/experiments/v167-workflow-execution-closure`,
  {
    method: "POST",
    headers: {
...
if ((!response.ok && response.status !== 422) || !payload.receipt) {
  throw new Error(
    payload.error || `v1.6.7 acceptance returned HTTP ${response.status}.`,
  );
}
...
writeFileSync(
  ...
  `${JSON.stringify(
    {
      schemaVersion: "workflows.execution-closure-runner-evidence.v1",
