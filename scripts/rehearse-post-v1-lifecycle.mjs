...
async function post(pathname) { return json(pathname, { method: "POST", headers: { "content-type": "application/json", ...(operatorToken ? { "x-first-llm-operator-key": operatorToken } : {}) }, body: "{}" }); }
...
const ok = aggregate.totals.ready === aggregate.totals.slices;
console.log(JSON.stringify({ ok, reportPath, totals: aggregate.totals }, null, 2));
