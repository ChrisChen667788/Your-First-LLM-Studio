...
| --- | --- | --- |
| Agent Workbench | `components/agent/AgentWorkbench.tsx` is roughly 8.9k lines. | Main shell, state orchestration, UI layout, compare wiring, session behavior, and runtime concerns are too close together. |
...
- Normalize timeline events across session, compare, benchmark, fine-tune, retrieval, model install, and provider health.
- Add report/artifact references to timeline events.
- Add cross-links from Fine-tune adapter -> Compare run -> Benchmark run -> Report bundle.
...
| v0.4.0-2026-06 | Product IA refactor | Foreground Fine-tune, Compare, Models, Benchmarks routes; admin narrowed to ops/config/audit. |
...
- Compare physical extraction now covers the prompt composer, lane matrix, review drawer, lane preview, recipe matrix/gallery, and execution handoff under `features/compare/components/*`; old `components/agent/compare/*`, `components/agent/AgentCompareLab.tsx`, and `components/agent/AgentRecipeGallery.tsx` paths are compatibility re-exports.
- Foreground `/fine-tune`, `/compare`, `/models`, and `/benchmarks` routes are live as product IA shells. `/compare` now enters through `features/compare/CompareRouteWorkbench.tsx` and no longer borrows `AgentWorkbench`, `/models` enters through `features/models/ModelsStudioShell.tsx`, `/benchmarks` enters through `features/benchmark/BenchmarksStudioShell.tsx` with prompt run controls, and `/admin` remains the monitoring/configuration mirror while deeper runner services continue to move forward.
- Compare reducer state, workbench implementation, composer/review/lane components, workbench state model, workbench orchestration model, action hook, lifecycle polling, persisted preferences/hydration, preference persistence model, reproduce artifacts, review helpers, recipe persistence, recipe apply/run orchestration, route shell, route-owned target sync/preferences, embedded Agent Compare adapter, workbench render portal/sidebar, dynamic loading shell, and Compare workbench prop adapter now live under `features/compare/*`; `components/agent/AgentCompareLab.tsx`, `components/agent/AgentRecipeGallery.tsx`, and `components/agent/compare/*` are only compatibility re-exports, and `AgentWorkbench.tsx` delegates embedded Compare orchestration, shell props, Compare preference input persistence, and Compare reproduce request artifacts to feature ports.
...
After that, use the route/module ownership matrix to choose the first safe extraction. The lowest-risk first extraction is Fine-tune, because it already has a clear workflow model from the LLaMA-Factory parity audit:
...
5. Chat Adapter
6. Export
7. Reports and bundles

The next extraction should keep shrinking generic Agent session sync and session export/import around Agent-owned chat state, while Benchmark can continue moving result orchestration and runtime/control policies behind smaller service ports and Fine-tune workflow contracts become the source of tab routing.
