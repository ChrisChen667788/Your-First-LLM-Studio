...
function buildFineTuneMarkdownReport(input: {
  ...
  return [
    ...
    `- Target modules: ${(recipe?.targetModules || plan?.targetModules || []).join(", ") || "--"}`,
    `- Scheduler / warmup: ${recipe?.scheduler || plan?.scheduler || "--"} / ${recipe?.warmupRatio ?? plan?.warmupRatio ?? "--"}`,
    `- Packing policy: ${recipe?.packingPolicy || plan?.packingPolicy || "--"}`,
    ...
    `- Eval / save cadence: ${recipe?.evalEverySteps ?? plan?.stepsPerEval ?? "--"} / ${recipe?.saveEverySteps ?? plan?.saveEvery ?? "--"} steps`,
    `- Best checkpoint metric: ${recipe?.bestCheckpointMetric || plan?.bestCheckpointMetric || "--"}`,
    `- Load best checkpoint at end: ${(recipe?.loadBestCheckpointAtEnd ?? plan?.loadBestCheckpointAtEnd) ? "yes" : "no"}`,
    `- Total planned steps: ${plan?.totalSteps ?? "--"}`,
    job.bestCheckpoint
      ? `- Selected checkpoint: step ${job.bestCheckpoint.step}, ${job.bestCheckpoint.metric} ${formatReportNumber(job.bestCheckpoint.value)}, ${job.bestCheckpoint.path || job.outputDir}`
      : "- Selected checkpoint: --",
    `- Saved checkpoint events: ${job.checkpointEvents?.length || 0}`,
    "",
    "## Loss Summary",
    "",
