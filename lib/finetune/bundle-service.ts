...
export function buildFineTuneBundleReadme(input: {
  ...
  return [
    ...
    `- Target modules: ${recipe.targetModules.join(", ")}`,
    `- Scheduler / warmup: ${recipe.scheduler} / ${recipe.warmupRatio}`,
    `- Packing policy: ${recipe.packingPolicy}`,
    `- Gradient accumulation: ${recipe.gradientAccumulationSteps}`,
    `- Validation split: ${recipe.validationSplitPct}%`,
    `- Eval / save cadence: ${recipe.evalEverySteps} / ${recipe.saveEverySteps} steps`,
    `- Best checkpoint: ${recipe.bestCheckpointMetric}, load at end ${recipe.loadBestCheckpointAtEnd ? "yes" : "no"}`,
    "",
